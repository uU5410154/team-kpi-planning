...
const moved = run({ assignee: 'Nakittapon Imjaijaroenying' })
check('THE PIC FOLLOWS THE TICKET', moved.projects[0].pic === 'pol', String(moved.projects[0].pic))
check('  and the sync says who it moved, and from what name',
  moved.reassigned === 1 && moved.reassignments[0].from === 'gun'
  && moved.reassignments[0].to === 'pol' && moved.reassignments[0].name === 'Nakittapon Imjaijaroenying',
  JSON.stringify(moved.reassignments))
check('  and the credited share goes with it',
  moved.projects[0].contributors.some((c) => c.person === 'pol')
  && !moved.projects[0].contributors.some((c) => c.person === 'gun'),
  JSON.stringify(moved.projects[0].contributors))
check('  keeping the role the work was being done in',
  moved.projects[0].contributors.find((c) => c.person === 'pol').roles.join() === 'pm')

check('AN UNRECOGNISED NAME CHANGES NOTHING',
  run({ assignee: 'Somebody Else' }).projects[0].pic === 'gun')
check('  and is not reported as a move', run({ assignee: 'Somebody Else' }).reassigned === 0)
...
const after = run({ assignee: 'Nakittapon Imjaijaroenying' }).projects[0]
 ...
 */
const held = { ...project, pic: 'james', contributors: [{ person: 'james', roles: ['pm'] }] }
const handed = mergeJira(
  state([held]),
  { issues: [issue({ assignee: 'Nakittapon Imjaijaroenying' })], epics: [], rollups: {} },
  { addNew: false },
).projects[0]
const before = computePlan(repairState(state([held])))
