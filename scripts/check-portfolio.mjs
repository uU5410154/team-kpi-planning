...
/*
 * The book, less the rows IT or the business owns AND less the rows nobody is
 * PIC of. The lead carries what the team is actually holding; a project with
 * no owner is on nobody's card, including the lead's.
 */
check('lead still equals the book less outside-owned and unowned',
  Math.abs(leadA.scorecardHours
    - (edited.totals.totalHours - edited.totals.outsideHours - edited.totals.unownedHours)) < 0.01,
  `${Math.round(leadA.scorecardHours)} vs ${Math.round(edited.totals.totalHours)} − ${Math.round(edited.totals.outsideHours)} − ${Math.round(edited.totals.unownedHours)}`)
